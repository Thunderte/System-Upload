...
<head>
    ...
    <title>Sunset: Meus Uploads</title>
    <script>
...
<div class="flex flex-col h-screen justify-between">
        ...
        <div class="container flex flex-wrap justify-between items-center mx-auto">
            ...
            <div class="hidden w-full md:block md:w-auto" id="navbar-default">
                <ul class="flex flex-col p-4 mt-4 rounded-lg border md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 bg-gray-800 md:bg-gray-900 border-gray-700">
                    <li>
                        <a href="/index" class="block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">Início</a>
                    </li>
                    <li>
                        <a href="/upload" class="block py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Upload</a>
                    </li>
                    <li>
    ...
    <div class="flex justify-center ml-4 flex-wrap space-x-4">
        <?php $puxando = $dbh->prepare("SELECT * FROM uploads WHERE author = :author ORDER BY id");
        $puxando->bindParam(':author', $_SESSION['username']);
        $puxando->execute();
        while($news = $puxando->fetch(PDO::FETCH_ASSOC)){?>
            <a href=<?php echo $news['pasta'].$news['arquivo'];?>>
                <div class="mt-10 max-w-sm rounded-lg border shadow-md bg-gray-800 border-gray-700">
                    <img class="rounded-t-lg" src=<?php echo $news['pasta'].$news['arquivo'];?> alt="" />

                    <div class="p-5">

                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-white"><?= $news['arquivo']; ?></h5>

                        <p class="mb-3 font-normal text-gray-400">Pasta: <?= $news['pasta'];?> <br> Arquivo: <?= $news['arquivo'];?></p>
                    </div>
                </div>
            </a>
        <?php } ?>
...
<script src="/templates/hyzecms/js/flowbite.js"></script>
</body>
</html>